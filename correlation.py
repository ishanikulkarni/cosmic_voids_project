    ...
    for w in wiggles:
        for realization in range(0,num_realizations):
            ...
            if not os.path.exists(file2):
                ...
                data['Velocity'] = da.from_array(np.column_stack((np.zeros(len(gadget_format[0])), np.zeros(len(gadget_format[0])), np.zeros(len(gadget_format[0])))), chunks=(100,3)) 
                #transform.StackColumns
